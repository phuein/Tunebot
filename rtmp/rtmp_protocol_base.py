...
HEADERS = {}        # Remember previous headers, for reuse.
...
def header_decode(stream):
    ...
    if bits == 3:
        try:
            header.streamId     = HEADERS[str(channelId)].streamId
            header.bodyLength   = HEADERS[str(channelId)].bodyLength
        except:
            pass
        try:
            header.datatype     = HEADERS[str(channelId)].datatype
        except:
            pass
        try:
            header.timestamp = HEADERS[str(channelId)].timestamp
        except:
            pass
        return header
    ...
    if bits < 2:
        ...
        try:
            header.streamId     = HEADERS["previous"].streamId
        except:
            pass
    ...
    HEADERS["previous"]      = header      # Type 1 & 2.
    HEADERS[str(channelId)]  = header      # Type 3.
